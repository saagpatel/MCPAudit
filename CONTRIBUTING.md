...
| --- | --- | --- | --- |
| Portable behavior | `uv run pytest -p no:cacheprovider -q -m 'not skipif' --ignore=tests/test_repo_hygiene.py tests/` | 1,590 selected; 0 skipped | Python 3.11–3.13, Git, and a writable test root. This excludes tests that declare host, service, or platform prerequisites; unmarked Git-dependent tests remain selected. |
