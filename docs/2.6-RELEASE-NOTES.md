...
Publication authority here covers the tag, GitHub Release, and PyPI and MCP
Registry entries produced by the release workflow, bound to the exact landed
commit and tag supplied at dispatch. Stable Action-ref movement, deployment,
and any other external registry update each remain a separate decision and are
outside this release state. The release workflow still verifies the tag,
commit, artifact hashes, and OIDC boundaries at publication time.
