...
- MCP SDK 2.0 through the latest 2.x release is supported. The package now
  requires MCP SDK 2.x because that major version changes the connected-server
  API; the connector, MCP server, and stdio test fixture use the SDK 2 public
  surfaces (`Client`, `MCPServer`, and constructor-based low-level `Server`
  handlers).
