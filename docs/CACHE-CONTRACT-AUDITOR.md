...
protocol versions are unsupported and produce `UNKNOWN`; an unsupported
...
`UNKNOWN`. So does a malformed required list/read payload, including members
that fail the method-specific item shape in the pinned MCP SDK; neither
establishes a cache scope for refresh, reuse, or notification grading. This
bounded shape check does not claim full server correctness.
...
| Failed refresh | A client MAY serve stale after a re-fetch error | A causal, exact-key `refresh_error` following observable expiry or a validated invalidation permits stale use only until a later valid successful refresh supersedes that failed attempt |
